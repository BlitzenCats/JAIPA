...
class NetworkLogger:
    ...
    def parse_network_responses(self) -> Dict[str, Dict[str, Any]]:
    ...
    def get_response_body(self, request_id: str) -> Optional[str]:
        """Get the actual response body for a request
        ...
        try:
            ...
            if not response_body:
                logger.debug(f"Empty response from Network.getResponseBody for {request_id}")
                return None
            ...
            if body:
                ...
                logger.debug(f"Response body is empty/null for {request_id}")
                return None
                
        except Exception as e:
            logger.debug(f"Could not retrieve response body for {request_id}: {type(e).__name__}: {e}")
            return None
    ...
    def extract_chat_data(self, chat_url: str, timeout: int = 10) -> Optional[Dict[str, Any]]:
        ...
        logger.info(f"Monitoring network for chat ID: {chat_id}")
    ...
    def clear_captured_data(self) -> None:
        ...
        logger.debug("Cleared captured response data and request tracking")
