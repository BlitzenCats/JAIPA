...
class ChatNetworkParser:
    ...
    def _process_messages(
        ...
        # Extract chat memory from summary (to inject into first message)
        chat_memory = chat_data.get("summary", "") if chat_data else ""
        if chat_memory:
            logger.info(f"Found chat memory/summary: {len(chat_memory)} chars")
        ...
        for group in message_groups:
            jsonl_msg = ChatNetworkParser._convert_to_jsonl(
                ...
                chat_data,
                user_persona_name,
                chat_memory if is_first_message else None  # Only inject memory into first message
            )
    ...
    def _convert_to_jsonl(
        ...
        Args:
            ...
            chat_memory: Optional chat memory/summary to inject into extra field
        ...
        try:
            ...
            # Inject chat memory if provided (for first message only)
            if chat_memory:
                extra["memory"] = chat_memory
                logger.debug(f"Injected chat memory into extra field: {len(chat_memory)} chars")
