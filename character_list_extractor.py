...
class CharacterListExtractor:
    ...
    def __init__(self, browser_manager: BrowserManager, config: ScraperConfig):
        ...
        self.deleted_and_private_characters: List[str] = []  # Characters that are BOTH
        ...
        self.duplicate_characters_skipped: int = 0  # Duplicates returned by API across pages
    ...
    def extract_character_list(self) -> bool:
        ...
        try:
            ...
            total_deleted_and_private = len(self.deleted_and_private_characters)
            
            total_unique = len(self.characters_by_id)
            total_extracted = len(self.characters_by_id)  # Unique count
            
            # Calculate public count (total minus all non-public categories)
            total_public = total_unique - total_deleted - total_private - total_deleted_and_private
            
            # Log GUI-parseable progress info
            logger.info(f"[PROGRESS] Characters extracted: {total_unique}/{self.total_characters_expected}")
            logger.info(f"[PROGRESS] Breakdown: public={total_public}, deleted_only={total_deleted}, private_only={total_private}, deleted+private={total_deleted_and_private}")
            if self.duplicate_characters_skipped > 0:
                logger.info(f"[PROGRESS] API duplicates skipped: {self.duplicate_characters_skipped}")
            logger.info(f"[PROGRESS] Total chats expected: {self.total_chats_expected}")
            
            # Validate extraction
            if self.total_characters_expected > 0:
                # Account for duplicates in the API count
                expected_unique = self.total_characters_expected - self.duplicate_characters_skipped
                ...
                elif total_extracted == expected_unique:
                    logger.info(f"✓ Character count validation PASSED: {total_extracted} unique ({self.duplicate_characters_skipped} duplicates in API response)")
                else:
                    logger.warning(
                        f"⚠ Character count mismatch: extracted {total_extracted} "
                        f"but API reported {self.total_characters_expected} "
                        f"(includes {self.duplicate_characters_skipped} duplicates)"
                    )
            
            logger.info(f"[OK] Character list extraction complete: {total_unique} unique characters")
            
            # Log tracking info for GUI
            if self.deleted_characters:
                logger.info(f"[INFO] Deleted-only characters: {total_deleted}")
            if self.private_characters:
                logger.info(f"[INFO] Private-only characters: {total_private}")
            if self.deleted_and_private_characters:
                logger.info(f"[INFO] Deleted AND Private characters: {total_deleted_and_private}")
            if self.duplicate_characters_skipped > 0:
                logger.info(f"[INFO] Duplicate entries skipped (API returns same char on multiple pages): {self.duplicate_characters_skipped}")
    ...
    def _process_character_list_response(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            for char in characters:
                ...
                categories = []
                char_entry = f"{char_name} (ID: {char_id}) | https://janitorai.com/characters/{char_id}"
                
                if is_deleted and not is_public:
                    # Both deleted AND private - track in overlap list ONLY
                    self.deleted_and_private_characters.append(char_entry)
                    categories.append("deleted+private")
                elif is_deleted:
                    # Only deleted
                    self.deleted_characters.append(char_entry)
                    categories.append("deleted")
                elif not is_public:
                    # Only private
                    self.private_characters.append(char_entry)
                ...
                logger.debug(f"Added character: {char_name} ({chat_count} chats) [{category_str}]")
    ...
    def get_deleted_characters(self) -> Dict[str, Dict[str, Any]]:
        """Get all deleted-only characters (excludes those that are also private)
        ...
        deleted = {}
        for char_id, char_data in self.characters_by_id.items():
            if char_data.get("is_deleted", False) and char_data.get("is_public", True):
    ...
    def get_private_characters(self) -> Dict[str, Dict[str, Any]]:
        ...
        private = {}
        for char_id, char_data in self.characters_by_id.items():
            if not char_data.get("is_public", True) and not char_data.get("is_deleted", False):
                private[char_id] = char_data
        return private
    
    def get_deleted_and_private_characters(self) -> Dict[str, Dict[str, Any]]:
        """Get all characters that are BOTH deleted AND private
        ...
        both = {}
        for char_id, char_data in self.characters_by_id.items():
            if char_data.get("is_deleted", False) and not char_data.get("is_public", True):
                both[char_id] = char_data
        return both
